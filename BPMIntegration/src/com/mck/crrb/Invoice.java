...
public class Invoice extends SAPIntegration {
	...
	static TreeMap<SimulatePriceRowHeader, TreeMap<String, CreditRebillMaterial>> bucketizePriceMap(CorrectionRow[] invoices) {
		...
		for (; i < invoices.length; i++) {
			...
			SimulatePriceRowHeader key = new SimulatePriceRowHeader(); //new SimulatePriceRowHeader(invoices[i].getCustomerId(), sdf.format(invoices[i].getPricingDate()), ...);
			//TODO: Hydrate key as SimulatePriceRowHeader 
			String value = invoices[i].getMaterialId();
			CreditRebillMaterial creditRebillMaterial = new CreditRebillMaterial();
			//TODO: Hydrate creditRebillMaterial here
			
			//System.out.println("NameValuePair[" + i + "]: " + key + ", materialId[" + i + "]: " + value);
			
			TreeMap<String, CreditRebillMaterial> materialList = priceMap.get(key);
			...
			if (materialList == null) { // Key not in TreeMap - new key found
				...
				materialList.put(value, creditRebillMaterial);	// First material in list
				priceMap.put(key, materialList);
			}
			else { 	// Key already exists in TreeMap
				//materialList.put(value, value);
				materialList.put(value, creditRebillMaterial);
			}
