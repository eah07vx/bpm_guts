...
public class SalesHistory {
	...
	public TWList getSalesHistory(String invoiceURL, String curPriceURL, String histPriceURL, 
		...
		if (currentPriceLookupResp != null && currentPriceLookupResp.getCurrentPriceResp() != null && currentPriceLookupResp.getCurrentPriceResp().length > 0) {
			mergeResponse(currentPriceLookupResp, isCurrentCorrection);
		}
		if(sopDebug) {
	...
	private void mergeResponse(CurrentPriceResp currentPriceLookupResp, boolean isCurrentCorrection) throws Exception {
			...
			for (i = 0; i < corrRows.length; i++) {
				for (j = 0; j < cpRows.length; j++) {
					...
					for (k = 0; k < materials.length; k++) {
						if (corrRows[i].getCustomerId() == cpRows[j].getCustomerId()
								&& corrRows[i].getPricingDate().compareTo(cpRows[j].getPricingDate()) == 0
								&& corrRows[i].getMaterialId() == materials[k].getMaterialId()) {
	...
	private static TreeMap<NameValuePair<String, String>, ArrayList<String>> bucketizePriceMap(CorrectionRow[] invoices) {
		TreeMap<NameValuePair<String, String>, ArrayList<String>> priceMap = new TreeMap<NameValuePair<String, String>, ArrayList<String>>();
		int i = 0;		
		...
		for (; i < invoices.length; i++) {
			...
			ArrayList<String> materialList = priceMap.get(key);
			
			if (materialList == null) { // Key not in TreeMap - new key found
				materialList = new ArrayList<String>();
				materialList.add(value);	// First material in list
	...
	private static String prepCurrentPriceReq(SalesHistoryResp salesHistoryResp, int startIndex, int endIndex) {
	...
	/*
	private static String prepHistoricalPriceReq(SalesHistoryResp salesHistoryResp, int startIndex, int endIndex) {
	...
	private static String prepPriceReq(SalesHistoryResp salesHistoryResp, String containerName, int startIndex, int endIndex) {
		...
		TreeMap<NameValuePair<String, String>, ArrayList<String>> priceMap = bucketizePriceMap(salesHistoryResp.getInvoiceLookupResp());
		
		Map<String, Object> priceReqMap = new HashMap<String, Object>();
		List<Object> pricingRequests = new ArrayList<Object>();
		int i = 0;
		for (Map.Entry<NameValuePair<String, String>, ArrayList<String>> entry : priceMap.entrySet()) {
			...
			pricingReq.put("materialIds", entry.getValue().toArray());		// value in the priceMap entry has materials 
